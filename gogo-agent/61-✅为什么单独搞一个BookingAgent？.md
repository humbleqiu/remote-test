...
| --- | --- | --- |
| 核心工具 | ItineraryPlannerTool<br>（比价排序）、<br>destinationLiveTools<br>、天气/签证 MCP | BookingWriteTools<br>（<br>cancel_booking<br>）、<br>apiKeyTools |
| 特色机制 | PlanNotebook+PlanAwareThinkingHook<br>（任务清单+深度思考推进） | BookingPersistenceHook<br>（下单成功自动落库 + 推送支付事件） |
