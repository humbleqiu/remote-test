...
![image.png](assets/fa00491a04a2.png)
...
| --- | --- | --- |
| 模型 | strongModelWithThinking<br>（qwen3.7-max + 思考） | stableModel<br>（GLM-5.1，不思考） |
| maxIters | 15 | 8 |
| 工具 | 规划工具 + 天气/签证 MCP + 子代理们 | 仅<br>policyTools<br>+<br>itineraryReviewTools |
...
这里有一个很关键的工程决策：**方案 JSON 不作为参数传入，工具自己按当前用户从 Redis 里捞。** 规划 Agent 生成的多套方案往往是一大坨 JSON（航班、酒店、评分、指标……*）。如果让模型把这坨东西当参数传给审核工具，会撞两堵墙：一是****上下文爆炸****（这坨 JSON 要占大量 token），二是****J*****SON 易解析失败**（模型手工拼 JSON 经常拼错、截断）。
