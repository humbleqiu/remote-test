...
```text
// ItineraryPlanAgent 用 qwen3.7-max + thinking（12 元/百万 token）
.model(strongModelWithThinking)

// ItineraryReviewAgent 用 glm-5.1（6 元/百万 token）
.model(stableModel)
```
...
```text
// ItineraryReviewAgent 只有两类工具
toolkit.registration().tool(policyTools).apply();           // 查政策
toolkit.registration().tool(itineraryReviewTools).apply();  // 做审核
```
