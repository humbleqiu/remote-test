...
private PlatformCancelOutcome cancelOnPlatform(BookingType bizType, String externalOrderNo, ...) {
    ...
    return result.success()
            ? PlatformCancelOutcome.cancelled("平台取消请求已发送。")
            : PlatformCancelOutcome.failed(result.errorMsg());
}
```
...
| 维度 | 下单 | 取消 |
| --- | --- | --- |
| 编排者 | LLM（读 skill 自己拼命令） | Java 代码（<br>cancel_booking<br>亲自拼命令、调<br>ProcessBuilder<br>） |
