...
RAG接入详见：[96-✅接入RAG 知识库，让 Agent 拥有领域知识.md](96-✅接入RAG 知识库，让 Agent 拥有领域知识.md)
...
Spring 启动
  ↓
容器扫到 @Configuration RagKnowledgeConfig
  ↓
调用 corporateTravelPolicyKnowledge(Resource)
  ↓
  ├─ new DashScopeTextEmbedding(...)      // 只是拿到客户端句柄
  ├─ new SimpleKnowledge(InMemoryStore, embedding)     // 空的内存向量库
  ├─ reader.read(policyFile.toPath()).block()// ← 解析docx→List<Doc>
  └─ knowledge.addDocuments(docs).block()   // ← 对每个chunk做embedding
  ↓
返回 Knowledge，作为 Spring bean 缓存
  ↓
运行期：LLM 触发 RAG 时，调 embedding 把 query 也转向量，去 InMemoryStore 做 cosine 相似度 top-k
```
