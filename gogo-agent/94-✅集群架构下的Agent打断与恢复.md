...
public boolean interrupt(String sessionId) {
    ...
    boolean local = executionRegistry.interruptLocal(sessionId);
...
```text
// AgentExecutionRegistry.interruptLocal
public boolean interruptLocal(String sessionId) {
    Set<Agent> agents = localAgents.remove(sessionId);
    if (agents == null || agents.isEmpty()) {
        return false;   // 本节点没有 → 交给广播
    }
    boolean interrupted = false;
    for (Agent agent : agents) {
        agent.interrupt();                       // 翻转中断标志
        interrupted = true;
        // 关键补偿：优雅中断跳过 PostCallEvent，这里手动保存记忆
...
```text
// AgentInterruptBroadcast implements MessageListener
...
public void onMessage(Message message, byte[] pattern) {
    ...
    executionRegistry.interruptLocal(sessionId);   // 只有持有者会真正命中
}
